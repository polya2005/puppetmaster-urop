...
def generate_filename(num_objs_per_txn, write_prob, zipf_param, addr_space, num_txns, is_split):
...
def generate_all_workloads(output_dir="workloads", use_binary=False, use_split=False):
  ...
  for zipf_param, write_prob, num_objs in tqdm(cases):
    filename_base = generate_filename(num_objs, write_prob, zipf_param, ADDR_SPACE_SIZE, NUM_TXNS, use_split)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--csv", action="store_true", help="Generate .csv files instead of .bin")
  parser.add_argument("--split", action="store_true", help="Generate transactions with exact read/write split")
  parser.add_argument("--out", type=str, default="workloads", help="Output directory")
  args = parser.parse_args()

  generate_all_workloads(output_dir=args.out, use_binary=not args.csv, use_split=args.split)
