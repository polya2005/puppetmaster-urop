...
def read_binary_output(filename: str) -> dict[str, Any]:
    ...
    with open(filename, "rb") as f:
        ...
        for stage in stages:
            ...
            data[f"{stage}_throughput"] = {"times": times, "values": throughputs}
...
def plot_throughput_individual(
    data: dict[str, Any], stage: str, output_file: str
) -> None:
    """
...
def plot_latency_histogram(
    data: dict[str, Any], latency_type: str, output_file: str
) -> None:
    """
    ...
    centers_converted, counts, cdfs, unit_id = process_histogram_data(
        hist_data, unit_id
    )
    val, unit_str, unit_label = convert_time_with_unit(
        0, unit_id
    )  # Just to get unit strings
    ...
    if len(centers_converted) > 1:
        ...
        bin_width = min(
            [
                centers_converted[i + 1] - centers_converted[i]
                for i in range(len(centers_converted) - 1)
            ]
        )
        ax1.bar(
            centers_converted,
    ...
    with PdfPages(output_file) as pdf:
        ...
        for lt_key, lt_name in latency_types:
            ...
            if USE_MARKERS:
                ...
                for p in percentiles:
                    idx = next(
                        (i for i, cdf in enumerate(cdfs) if cdf >= p), len(cdfs) - 1
                    )
                    if idx < len(centers_converted):
                        x = centers_converted[idx]
                        y = cdfs[idx]
            ...
            if non_zero_indices:
                if centers_converted:
                    ...
                    if total_count > 0:
                        mean_val = (
                            sum(
                                centers_converted[i] * counts[i]
                                for i in range(len(centers_converted))
                            )
                            / total_count
                        )
                        ...
                        median_idx = next(
                            (i for i, cdf in enumerate(cdfs) if cdf >= 0.5),
                            len(cdfs) - 1,
                        )
                        median_val = (
                            centers_converted[median_idx]
                            if median_idx < len(centers_converted)
                            else 0
                        )

                        # Find 95th percentile
                        p95_idx = next(
                            (i for i, cdf in enumerate(cdfs) if cdf >= 0.95),
                            len(cdfs) - 1,
                        )
                        p95_val = (
                            centers_converted[p95_idx]
                            if p95_idx < len(centers_converted)
                            else 0
                        )

                        # Find 99th percentile
                        p99_idx = next(
                            (i for i, cdf in enumerate(cdfs) if cdf >= 0.99),
                            len(cdfs) - 1,
                        )
                        p99_val = (
                            centers_converted[p99_idx]
                            if p99_idx < len(centers_converted)
                            else 0
                        )
                        ...
                        variance = (
                            sum(
                                ((centers_converted[i] - mean_val) ** 2) * counts[i]
                                for i in range(len(centers_converted))
                            )
                            / total_count
                        )
                        ...
                        cv = (
                            (std_dev / mean_val * 100) if mean_val > 0 else 0
                        )  # as percentage
