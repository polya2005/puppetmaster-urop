...
static const bool complain_missing  = true;
...
static const double fraction_warmup_time = 0.0;
static const double fraction_cooldown_time = 0.0;
static const int num_throughput_windows = 30;

// Outlier removal configuration
static const double lower_percentile_cutoff = 0.000;
static const double upper_percentile_cutoff = 1.000; // makes sense to cut a lot more due to logging
...
{
  if (argc != 5) {
    fprintf(stderr, "Usage: %s transactions.{csv,bin} log.bin NUM_PUPPETS WORK_SIM_US\n", argv[0]);
    exit(1);
  }
  ...
  */
  ...
  const char *txn_file  = argv[1];
  const char *log_file  = argv[2];
  int num_puppets       = atoi(argv[3]);
  int work_sim_us       = atoi(argv[4]);

  workload_t *wl = parse_workload(txn_file);
  if (!wl) FATAL("Failed to parse %s", txn_file);
