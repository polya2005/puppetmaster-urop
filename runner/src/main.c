...
static const char *usage =
  ...
  "  --timeout SEC        Benchmark wall‑clock duration (default 60)\n"
  ...
  "  --limit              Limit the throughput of clients for latency measurement\n"
...
static void *client_thread(void *arg) {
  ...
  double safety_factor = 3;
  double expected_total_throughput = 1e7;
  uint64_t client_sim_cycles = work_sim_cycles * safety_factor;
  if (work_sim_cycles == 0) client_sim_cycles = (cpu_freq / expected_total_throughput) * safety_factor;
...
int main(int argc, char *argv[]) {
  ...
  // Sanity check
  INFO("Dumping the first few transactions for sanity check...");
  for (int i = 0; i < 5 && i < workload->num_txns; ++i) {
    dump_txn(stderr, &workload->txns[i]);
  }
