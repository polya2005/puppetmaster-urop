...
static atomic_int num_events = ATOMIC_VAR_INIT(0);
...
static void dump_event_human(FILE *dst, const pmlog_evt_t *e) {
  ...
  fprintf(dst, "[+%.8f] txn_id=%u %s", us, e->txn_id, kind_to_str(e->kind));
...
void pmlog_dump_text(FILE *f) {
  qsort(pmlog_evt_buf, num_events, sizeof(pmlog_evt_t), compare_events);
  fprintf(f, "num_events=%d\n", atomic_load(&num_events));
  fprintf(f, "cpu_freq=%.8f\n", cpu_freq);
  for (int i = 0; i < (int)atomic_load(&num_events); ++i) {
